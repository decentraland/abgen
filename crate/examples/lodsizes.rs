...
fn main() {
    ...
    for path in std::env::args().skip(1) {
        ...
        println!("== {short} ({} bytes on disk, unity {})", data.len(), bundle.version_engine);
        ...
        for f in &bundle.files {
            let FileContent::Serialized(sf) = &f.content else {
                if let FileContent::Raw(b) = &f.content { uncompressed += b.len(); println!("  raw file {} = {} bytes", f.name, b.len()); }
                continue;
            };
            let total: usize = sf.objects.iter().map(|o| o.data.len()).sum();
            uncompressed += total;
            println!("  serialized file {} objects={} object-bytes={}", f.name, sf.objects.len(), total);
            for obj in &sf.objects {
                ...
                if obj.class_id == 43 {
                    ...
                    let vcount = vd.and_then(|x| x.get("m_VertexCount")).and_then(|x| x.as_i64()).unwrap_or(-1);
                    let vbytes = vd.and_then(|x| x.get("m_DataSize")).and_then(|x| x.as_bytes());
                    let ib = v.get("m_IndexBuffer").and_then(|x| x.as_bytes());
                    let channels: Vec<String> = vd
                        .and_then(|x| x.get("m_Channels")).and_then(|x| x.as_array())
                        .map(|arr| arr.iter().enumerate().filter_map(|(i, c)| {
                            let dim = c.get("dimension")?.as_i64()?;
                            if dim == 0 { return None; }
                            Some(format!("ch{i}:dim{dim}/fmt{}/s{}", c.get("format")?.as_i64()?, c.get("stream")?.as_i64()?))
                        }).collect())
                        .unwrap_or_default();
                    println!(
                    ...
                    if let Some(dir) = &dump {
                        ...
                        if let Some(b) = vbytes { std::fs::write(format!("{dir}/{name}.vertex.bin"), b).unwrap(); }
                        if let Some(b) = ib { std::fs::write(format!("{dir}/{name}.index.bin"), b).unwrap(); }
                    }
                }
                if obj.class_id == 4 {
                    let v = sf.read_typetree(obj).expect("transform typetree");
                    let g = |k: &str, c: &str| v.get(k).and_then(|x| x.get(c)).and_then(|x| x.as_f64()).unwrap_or(f64::NAN);
                ...
                if obj.class_id == 28 {
                    ...
                    let img = v.get("image data").and_then(|x| x.as_bytes()).map(|b| b.len()).unwrap_or(0);
                    println!("    Texture2D {:?} obj={}B imagedata={img}B", v.get("m_Name").and_then(|x| x.as_str()).unwrap_or(""), obj.data.len());
                }
            }
        }
        println!("  by class: {}", by_class.iter().map(|(c, (n, b))| format!("{c}x{n}={b}B")).collect::<Vec<_>>().join("  "));
        println!("  uncompressed total={uncompressed}B  on-disk={}B  ratio={:.2}", data.len(), data.len() as f64 / uncompressed.max(1) as f64);
    }
}

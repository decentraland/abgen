...
/// `mime-types` Content-Type (`model/gltf-binary` for `.glb`); the file names
/// embed the content-addressed entity id. The abcdn edge deliberately serves
/// its JIT-regenerated ISS copies no-cache; the S3 objects match the
/// production writer.
...
mod tests {
    ...
    fn put_sends_key_derived_content_type_and_cache_control() {
        for (key, ct, cc) in [
            ...
            assert_eq!(value("content-encoding"), None, "{head}");
        }
    }

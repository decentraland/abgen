...
| `ABGEN_UPSTREAM_AB_REGISTRY` | derived | registry pass-through: records for entities without local bundles are answered with this registry's versions. Derived from `ABGEN_UPSTREAM_AB_CDN` as `asset-bundle-registry.<domain>` whenever the CDN is `ab-cdn.<domain>` (any domain); set it explicitly for other CDN shapes (gateway paths, buckets), or `off` to disable |
